...
from src.dns import DNSTwistChecker
from src.google import GoogleSafeBrowsingChecker
from src.logger import logger
from src.virust_total import VirusTotalChecker
...
class PhishingDetector:
    def __init__(
        self,
        db_path: str = "phishing_monitor.db",
        vt_api_key: str = None,
        ...
        self.gsb_checker = (
            GoogleSafeBrowsingChecker(gsb_api_key) if gsb_api_key else None
        )
    ...
    def init_database(self):
        ...
        if os.path.exists(self.db_path):
            try:
                ...
                cursor.execute(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='processed_certs'"
                )
    ...
    def _create_database(self):
        ...
        cursor.execute(
            "CREATE INDEX idx_processed_certs_cert_id ON processed_certs(cert_id)"
        )
        cursor.execute(
            "CREATE INDEX idx_processed_certs_domain ON processed_certs(domain)"
        )
        cursor.execute(
            "CREATE INDEX idx_phishing_alerts_detected_at ON phishing_alerts(detected_at)"
        )
        cursor.execute(
            "CREATE INDEX idx_phishing_alerts_risk_score ON phishing_alerts(risk_score)"
        )
        cursor.execute(
            "CREATE INDEX idx_virustotal_checks_domain ON virustotal_checks(domain)"
        )
        cursor.execute(
            "CREATE INDEX idx_dnstwist_results_original ON dnstwist_results(original_domain)"
        )
    ...
    def is_cert_processed(self, cert_id: int) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "SELECT 1 FROM processed_certs WHERE cert_id = ?", (cert_id,)
            )
    ...
    def check_with_google_safe_browsing(self, domain: str) -> Dict:
        ...
        if not self.gsb_checker:
            return {
                "found": False,
                "error": "Google Safe Browsing API key not provided",
            }
    ...
    def analyze_certificate_enhanced(self, cert: Dict) -> Dict:
        ...
        analysis = {
            ...
            "phishtank_threat": phishtank_result.get("found", False)
            if phishtank_result
            else False,
    ...
    def generate_enhanced_alert(self, analysis: Dict):
        ...
        logger.warning(
            f"   ENHANCED PHISHING ALERT - Risk Score: {analysis['risk_score']}"
        )
        ...
        if analysis["suspicious_patterns"]:
            logger.warning(
                f"   Suspicious Patterns: {len(analysis['suspicious_patterns'])} detected"
            )
        ...
        if vt_result:
            if vt_result.get("found"):
                ...
                total = vt_result.get("total", 0)
                logger.warning(
                    f"   VirusTotal: {positives}/{total} engines flagged as malicious"
                )
                if vt_result.get("permalink"):
                    logger.warning(f"   VirusTotal Report: {vt_result['permalink']}")
            else:
                logger.warning("   VirusTotal: Not found in database")

        # Google Safe Browsing
        if analysis.get("gsb_threat"):
            logger.warning("   Google Safe Browsing: THREAT DETECTED")
        else:
            logger.warning("   Google Safe Browsing: Clean")

        # PhishTank
        if analysis.get("phishtank_threat"):
            logger.warning("   PhishTank: KNOWN PHISHING SITE")
        else:
            logger.warning("   PhishTank: Not in database")

        # DNSTwist
        variants_count = analysis.get("typosquatting_variants", 0)
        if variants_count > 0:
            logger.warning(
                f"   Typosquatting Variants: {variants_count} similar domains found"
            )
    ...
    def run_enhanced_scan(self):
        ...
        logger.info(
            f"Google Safe Browsing: {'Enabled' if self.gsb_checker else 'Disabled'}"
        )
        logger.info(
            f"DNSTwist: {'Enabled' if self.dnstwist_checker.dnstwist_available else 'Disabled'}"
        )
        ...
        logger.info(
            f"Enhanced scan complete: {new_certs} new certificates, {suspicious_certs} suspicious"
        )
            ...
            for alert in all_alerts[:10]:
                ...
                if alert.get("vt_positives", 0) > 0:
                    threat_indicators.append(
                        f"VT:{alert['vt_positives']}/{alert['vt_total']}"
                    )
                ...
                logger.info(
                    f"   {alert['domain']} (Score: {alert['risk_score']}){indicators_str}"
                )
    ...
    def monitor_certificates_enhanced(self, interval: int = 300):
        ...
        while True:
            try:
                ...
                logger.info(
                    f"Processed {new_certs} new certificates, {suspicious_certs} suspicious"
                )
